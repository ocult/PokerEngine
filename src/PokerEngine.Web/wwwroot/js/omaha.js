const omahaForm = document.querySelector('#omaha-form');
const gameId = omahaForm.querySelector('#gameid');
const phase = omahaForm.querySelector('#phase');
...
function renderOmahaError(message) {
...
function renderOmahaSuccess(payload) {
  gameId.value = payload.gameId ?? '';
  if (payload.phase === 'Complete') {
    gameId.value = '';
  }
  omahaForm.querySelector('button[type="submit"]').textContent = gameId.value ? 'Continue' : 'Next Hand';
...
if (omahaForm) {
  omahaForm.addEventListener('submit', async (event) => {
    event.preventDefault();

    const players = document.querySelector('#players').value;
    const gameid = gameId.value;
    const response = await fetch('/api/omaha-holdem', {
      method: 'POST',
    ...
    if (!response.ok) {
      renderOmahaError(payload.error ?? 'Fail to execute the Omaha Hold\'em.');
      return;
    }

    document.querySelector('#players').setAttribute('disabled', 'disabled');
    renderOmahaSuccess(payload);
  });
}
